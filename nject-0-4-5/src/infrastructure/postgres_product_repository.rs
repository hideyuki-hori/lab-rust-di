...
pub(crate) struct PostgresProductRepository {
...
mod integration_tests {
    ...
    use crate::domain::value_objects::{
        Price, ProductDescription, ProductId, ProductName, Quantity,
    };
    use crate::interface::product_repository::ProductRepository;
    ...
    async fn update_stock_decreases() {
        ...
        repository.update_stock(product.id, -3).await.unwrap();
    ...
    async fn update_stock_not_found() {
        ...
        let result = repository.update_stock(ProductId::new(), -1).await;
