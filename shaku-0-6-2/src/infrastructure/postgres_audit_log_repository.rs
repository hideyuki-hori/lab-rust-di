...
#[derive(Component)]
#[shaku(interface = AuditLogRepository)]
...
impl AuditLogRepository for PostgresAuditLogRepository {
    async fn create(&self, audit_log: &AuditLog) -> Result<AuditLog, AppError> {
        let created =
            sqlx::query_as::<_, AuditLog>(include_str!("sql/audit_logs/create.sql"))
                .bind(audit_log.id)
    ...
    async fn find_all(&self) -> Result<Vec<AuditLog>, AppError> {
        let logs =
            sqlx::query_as::<_, AuditLog>(include_str!("sql/audit_logs/find_all.sql"))
                .fetch_all(&self.pool)
                .await?;
        Ok(logs)
    }
